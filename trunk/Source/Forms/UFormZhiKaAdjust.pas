...
uses
  ...
  cxMaskEdit, cxButtonEdit, cxLabel, Menus, cxRadioGroup;
...
begin
  ...
  nStr := 'Select * From %s Where Z_Custom=''%s'' and ' +
...
begin
  ...
  begin
    ...
    nStr := 'Update $ZC Set C_ZID=''$NewZK'' Where C_ZID=''$ID''';
    nStr := MacroValue(nStr, [MI('$ZC', sTable_ZhiKaCard),
            MI('$ID', gZKItems[ListZK.ItemIndex].FZhiKa)]);
    gInfo.FSQLRes := gInfo.FSQLRes + #13#10 + nStr;
  end;
