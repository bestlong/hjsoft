...
begin
  ...
  nSQL := 'Select L_SaleMan,L_Custom,L_Type,L_Stock,L_Price,' +
          'Sum(L_Value) as L_Value From $Bill ' +
          'Where L_IsDone=''$Yes'' And L_Custom In (Select T_SysID From $Tmp) ' +
          'Group By L_SaleMan,L_Custom,L_Type,L_Stock,L_Price';
  nSQL := MacroValue(nSQL, [MI('$Bill', sTable_Bill), MI('$Yes', sFlag_Yes),
          MI('$Tmp', sTable_DataTemp)]);
  //同客户同品种同单价合并

  nStr := 'Select ''$W'' As R_Week,''$Man'' As R_Man,$Now As R_Date,b.*,' +
          'S_Name,C_Name From ($Bill) b ' +
