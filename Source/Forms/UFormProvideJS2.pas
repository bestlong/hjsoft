...
type
  TfFormProvideJS2 = class(TfFormNormal)
...
class function TfFormProvideJS2.CreateForm(const nPopedom: string;
...
begin
  ...
  with TfFormProvideJS2.Create(Application) do
  begin
    Caption := '批量核算';
...
class function TfFormProvideJS2.FormID: integer;
begin
  Result := cFI_FormProvideHS_P;
end;

procedure TfFormProvideJS2.InitFormData(const nID: string);
begin
  EditYF.Text := '0';
  EditPrice.Text := '0';
  EditMoney.Text := '0';
...
//Desc: 计算金额
procedure TfFormProvideJS2.EditPricePropertiesEditValueChanged(
  Sender: TObject);
begin
...
//------------------------------------------------------------------------------
//Desc: 验证数据
function TfFormProvideJS2.OnVerifyCtrl(Sender: TObject; var nHint: string): Boolean;
begin
  Result := True;
...
//Desc: 保存
procedure TfFormProvideJS2.BtnOKClick(Sender: TObject);
...
initialization
  gControlManager.RegCtrl(TfFormProvideJS2, TfFormProvideJS2.FormID);
end.
